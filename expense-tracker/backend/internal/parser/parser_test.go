...
func TestBuildObjectName_Basic(t *testing.T) {
	name, err := BuildObjectName("2026-09-01", "Starbucks", 4.5, "jpg")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.HasPrefix(name, "receipts/2026-09-01_starbucks_4.50_") {
		t.Fatalf("got %q, want prefix receipts/2026-09-01_starbucks_4.50_", name)
	}
	if !strings.HasSuffix(name, ".jpg") {
		t.Fatalf("got %q, want .jpg suffix", name)
	}
	// receipts/2026-09-01_starbucks_4.50_<6char>.jpg
	base := strings.TrimSuffix(strings.TrimPrefix(name, "receipts/"), ".jpg")
	parts := strings.Split(base, "_")
	if len(parts) != 4 {
		t.Fatalf("expected 4 underscore parts, got %d in %q", len(parts), base)
	}
	if len(parts[3]) != 6 {
		t.Fatalf("expected 6-char shortid, got %q", parts[3])
	}
}

func TestBuildObjectName_SlugifiesVendor(t *testing.T) {
	name, err := BuildObjectName("2026-09-01", "Joe's  Café & Grill!", 12.0, "png")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildObjectName_Validation(t *testing.T) {
	cases := []struct {
		...
		{"bad date", "2026/09/01", "jpg", "v", 1},
		{"empty date", "", "jpg", "v", 1},
		{"empty ext", "2026-09-01", "", "v", 1},
		{"empty vendor", "2026-09-01", "jpg", "", 1},
		{"negative amount", "2026-09-01", "jpg", "v", -1},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if _, err := BuildObjectName(c.date, c.vendor, c.amount, c.ext); err == nil {
...
func TestParse_Basic(t *testing.T) {
	...
	if e.SourceObject != "receipts/2026-09-01_starbucks_4.50_a1b2c3.jpg" {
		t.Errorf("source_object = %q", e.SourceObject)
	}
}

func TestParse_LargeAmount(t *testing.T) {
	e, err := Parse("receipts/2026-09-01_acme_1234.50_zzz999.pdf")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestRoundTrip(t *testing.T) {
	name, err := BuildObjectName("2026-12-31", "Whole Foods Market", 99.99, "jpeg")
	if err != nil {
...
func TestParse_BadDateFallback(t *testing.T) {
	...
	e, err := Parse("receipts/notadate_starbucks_4.50_a1b2c3.jpg")
	if err != nil {
		t.Fatalf("expected lenient parse, got error: %v", err)
	}
	if e.Vendor != "starbucks" {
